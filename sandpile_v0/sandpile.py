...
class Sandpile2():          ### Define the class Sandpile2
    ...
    ### Initializing other data
        ...
        # self.adjacency = [ [len(self.graph.edges(vertices=[y], labels=False)) for y in self.v_vertices ] for x in self.v_vertices ]
    ...
    def is_recurrent(self, config=[], virtual=True):
        r"""
        ...
        """
        start_conf = copy.copy(self.current_conf)                                           # The configuration before computations
        
        if config != []:
            self.current_conf = copy.copy(config)
    ...
    def minimal_conf(self, configs):
        ...
        """
        ### Delete bad configs
        print("Il numero di configurazioni iniziale è {}".format(len(configs)))
        count = 0
        elim = 0
        for i in configs:
            j = 0
            ...
            while j < self.v_len and check == False:
                if i[j] >= self.out_degree[j]:
                    #print("Eliminata configurazione {}".format(i))
                    configs.pop(count)
                    check = True
                    count -= 1
                    elim +=1
        ...
        ### Delete duplicates
        print("Il numero di configurazioni stabili è {}, eliminate {}".format(len(configs), elim))
        configs.sort()
        list(configs for configs,_ in itertools.groupby(configs))
        print("Il numero di configurazioni stabili senza ripetizioni è {}".format(len(configs)))

        ### Delete non-minimal
        count = 0
        iterat = 0
        index = 0
        while index < len(configs):                             # For each configuration...
            iterat += 1
            print("Iterazione {} con totale {}".format(iterat, len(configs)))
            i = 0
            cond = True                              
            while i < index and cond:                          # ...check the previous ones and delete those greater
                geq = True    # condition for i < current
                leq = True    # condition for i > current
                j = 0
                while j < self.v_len:
                    if configs[i][j] < configs[index][j]:
                        geq = False
                    else:
                        leq = False
                    j += 1
                if geq == True and leq == False:                # i >> current
                    configs.pop(i)
                elif geq == False and leq == True:              # i << current
                    configs.pop(index)
                    cond = False
                    index -= 1
                else:
    ...
    def recurrent_conf(self, minimal=False, check=False):
        r"""
        ...
        while depth >= 0:                           # Continue the search depth-first until we checked everything
            ...
            if len(branchings[depth][3]) == 0:          # There is no more good candidate...
                if depth == self.v_len - 1:                     # ...because we have a good configuration! (USELESS CASE, I THINK)
                    ...
                    if check:
                        current = copy.copy(branchings[depth][4])
                        if (self.is_recurrent(current))[0] == False:
                            print("NONEEEE {}".format(current))
                ...
                self.current_conf = self.topple_vertex(new_cand)  
