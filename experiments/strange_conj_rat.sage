    ...
    while toppl != finalv:                               # Until everything has been toppled k times...
        for i in range(len(order)):
            ...
            if (sandp.sandpile_struct.out_degree(order[i]) <= sandp.sandpile_config[order[i]]) and (toppl[i] == 0):
                ...
                print("Toppl word {} and finalv {} for config {} and vertex {} with restr {} and n {} and increase {}".format(toppl,finalv,sandp.sandpile_config,visit_vertex, restr, n, incr))
                delay += incr
            else:
...
print("CONJECTURE: why is it always the inverse??? I think this delay counts coarea...")
print("\nIs it symmetric?")
